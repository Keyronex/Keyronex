...
{
	size_t size = roundup2(highest, 8) / 8;
...
idalloc_alloc(struct id_allocator *alloc)
{
	...
	ipl = ke_spinlock_enter(&alloc->lock);
	...
	for (unsigned int i = 0; i <= alloc->max; i++) {
		...
		if (!BIT_ISSET(alloc->bitmap, index)) {
			...
			alloc->rotor = index;
			ke_spinlock_exit(&alloc->lock, ipl);
			return index;
		}
	}
	ke_spinlock_exit(&alloc->lock, ipl);
...
{
	ipl_t ipl = ke_spinlock_enter(&alloc->lock);
...
{
	kmem_free(alloc->bitmap, roundup2(alloc->max, 8) / 8);
}
