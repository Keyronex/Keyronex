...
struct vm_slab_page {
	uint64_t pmm_flags;
	struct kmem_slab slab;
	char padding[sizeof(vm_page_t) - sizeof(uint64_t) -
    ...
    size_t align, void (*ctor)(void *))
{
	...
	ipl = ke_spinlock_enter(&all_caches_lock);
	TAILQ_INSERT_TAIL(&all_caches, cache, qlink);
	ke_spinlock_exit(&all_caches_lock, ipl);
}
...
zonenum(size_t size)
{
	kassert(size > 0, "kmem_xalloc: zero-size allocation");
	if (size <= 8)
		return 0;
	if (size <= 16)
		return 1;
...
void *
kmem_xalloc(size_t size, vm_alloc_flags_t flags)
{
	...
	if (zoneidx != -1)
		...
		return vm_kwired_alloc(roundup2(size, PGSIZE) / PGSIZE, flags);
}

void *
kmem_xzalloc(size_t size, vm_alloc_flags_t flags)
{
...
kmem_xfree(void *ptr, size_t size)
{
	...
	if (zoneidx != -1)
		...
		vm_kwired_free(ptr, roundup2(size, PGSIZE) / PGSIZE);
}

static struct kmem_slab *
small_slab_new(kmem_cache_t *cache, vm_alloc_flags_t flags)
{
	vm_page_t *page;
	...
	page = vm_page_alloc(VM_PAGE_KWIRED, 0, VM_DOMID_ANY, 0);
	if (page == NULL)
		return NULL;

	slab_page = (struct vm_slab_page *)page;
	slab = &slab_page->slab;
	base = vm_page_hhdm_addr(page);
...
static struct kmem_slab *
large_slab_new(kmem_cache_t *cache, vm_alloc_flags_t flags)
{
	struct kmem_slab *slab;
	void *base;

	base = vm_kwired_alloc(slab_size(cache) / PGSIZE, flags);
	if (base == NULL)
		return NULL;
...
static void *
kmem_slablayer_alloc(kmem_cache_t *cache, vm_alloc_flags_t flags)
{
	struct kmem_slab *slab;
	struct kmem_bufctl *bufctl;
	void *ret;

	ke_spinlock_enter_nospl(&cache->spinlock);
	...
	if (slab == NULL || slab->free_n == 0) {
		...
		if (unlikely(slab == NULL)) {
			ke_spinlock_exit_nospl(&cache->spinlock);
			return NULL;
		}
	...
	kassert(slab->free_n + slab->alloced_n == slab_elems_per(cache),
	    "slab free+alloced count mismatch");
	if (cache->size <= SMALL_SLAB_MAX) {
		kassert((void *)bufctl >= (void *)HHDM_BASE &&
		    (void *)bufctl < (void *)(HHDM_BASE + HHDM_SIZE),
		    "small slab bufctl not in HHDM");
	} else {
		kassert(bufctl->base >= (void *)PIN_HEAP_BASE &&
		    bufctl->base < (void *)(PIN_HEAP_BASE + PIN_HEAP_SIZE),
		    "large slab bufctl base not in wired range");
		kassert(bufctl->slab == slab,
		    "large slab bufctl slab mismatch");
	}
#endif
	...
	if (slab->free_n == 0) {
		...
		kassert(SLIST_EMPTY(&slab->free),
		    "slab free list empty but free_n is not zero");
#endif
...
kmem_slablayer_free(kmem_cache_t *cache, void *ptr)
{
	...
	if (likely(cache->size <= SMALL_SLAB_MAX)) {
		...
		kassert(ptr >= (void *)HHDM_BASE && ptr < (void *)(HHDM_END),
		    "small cache ptr not in HHDM");
#endif
		page = VM_PAGE_FOR_HHDM_ADDR((vaddr_t)ptr);
#if KMEM_SANITY_CHECKING
...
kmem_vasprintf(char **strp, const char *fmt, va_list ap)
{
	...
	va_copy(apcopy, ap);
	size = kvsnprintf(NULL, 0, fmt, apcopy);
	va_end(apcopy);
	...
	*strp = (char *)kmem_xalloc(size + 1, 0);
	...
	size = kvsnprintf(*strp, size + 1, fmt, ap);
	return size;
}
    ...
    vm_alloc_flags_t flags)
{
	...
	ke_spinlock_enter_nospl(&cp->depot.kd_lock);
	...
	ke_spinlock_exit_nospl(&cp->depot.kd_lock);
...
void *
kmem_magazine_alloc(kmem_cache_t *cp, vm_alloc_flags_t flags)
{
	struct kmem_cpu_cache *ccp;
	void *obj;

	ccp = &cp->cache_cpu[CPU_LOCAL_LOAD(cpu_num)];
...
magazine_layer_init(kmem_cache_t *cp, size_t magsize)
{
	...
	cp->cache_cpu = kmem_xalloc(sizeof(struct kmem_cpu_cache) * ke_ncpu, 0);
	cp->cache_magsize = magsize;

	for (int i = 0; i < ke_ncpu; i++) {
...
void *
kmem_cache_alloc(kmem_cache_t *cache, vm_alloc_flags_t flags)
{
	void *ret;
	ipl_t ipl;
