 ...
 * just for virtual memory, despite its name.
 *
 * Keyronex VMem is an implementation of the allocator described in Adams and
 * Bonwick (2001) as used in Solaris.
 * The code is original.
 *
 * ## Arena
 *
 * An arena is an interval characterised by its start point, end point, and
 * quantum. For example, the arena kernel_heap starts at KHEAP_START, ends at
 * KHEAP_END, and has a quantum of PGSIZE; the quantum is the natural unit of an
 * arena, and allocations are made in multiples of it and aligned to it.
 *
 ...
 * ## Span
 *
 * A span is an interval within an arena; they made be imported from a backing
 * source arena, or explicitly given over to an arena's control. They must not
 * overlap.
 *
 * ## Segment
 *
...
/** Allocate a segment. */
static vmem_seg_t *
seg_alloc(vmem_t *vmem, vm_alloc_flags_t flags)
{
	return kmem_cache_alloc(seg_cache, flags);
}
...
{
	return kmem_cache_free(seg_cache, seg);
}
...
{
	uintptr_t hash = murmur64(addr) % VMEM_HASHBUCKET_N;
...
vmem_add_internal(vmem_t *vmem, int spantype, vmem_addr_t base,
    vmem_size_t size, vm_alloc_flags_t flags, vmem_seg_t **freeseg_out)
{
...
vmem_init(vmem_t *vmem, const char *name, vmem_addr_t base, vmem_size_t size,
    ...
    vmem_t *source, size_t qcache_max, vm_alloc_flags_t flags)
{
	...
	for (int i = 0; i < VMEM_FREELIST_N; i++)
		LIST_INIT(&vmem->freelist[i]);
	for (int i = 0; i < VMEM_HASHBUCKET_N; i++)
...
int
vmem_add(vmem_t *vmem, vmem_addr_t base, vmem_size_t size,
...
{
	kassert(seg->type == kVMemSegFree, "vmem seg not free");
...
static int
try_import(vmem_t *vmem, vmem_size_t size, vm_alloc_flags_t flags,
    vmem_seg_t **out)
{
	vmem_addr_t addr;
	int r;
    ...
    vm_alloc_flags_t flags, vmem_addr_t *out)
{
	...
	kassert(min == 0 || (flags & VM_EXACT), "not supported yet");
	kassert(max == 0," not supported yet\n");

	kassert(size != 0, "size must not be 0");
...
search:
	...
	LIST_FOREACH(freeseg, list, seglist) {
		if (freeseg->base <= min) {
			addr = min;
		} else if (flags & VM_EXACT && freeseg->base > min) {
			continue;
		} else {
			addr = freeseg->base;
		}
    ...
    vm_alloc_flags_t flags)
{
	...
	kfatal("vmem_xfree: segment at address 0x%zx doesn't exist\n", addr);
	return -1;

free:
	if (size != 0 && seg->size != size)
		kfatal("vmem_xfree: size mismatch (got 0x%zx, actual 0x%zx)\n",
		    size, seg->size);
    ...
    vmem_size_t newsize, vm_alloc_flags_t flags, vmem_addr_t *out)
{
	...
	if (oldsize == 0) {
		kassert(newsize != 0, "can't turn zero to zero");
	...
	if (seg == NULL) {
		kfatal("vmem_xrealloc: no segment at 0x%zx sized 0x%zx\n",
		    addr, oldsize);
		return -1;
	}
...
vmem_global_init()
{
	seg_cache = kmem_cache_create("vmem_seg", sizeof(vmem_seg_t),
	    _Alignof(vmem_seg_t), NULL);
}
