...
DKLogAttach(DKDevice *child, DKDevice *parent)
{
	...
	kdprintf(kAnsiYellow "%s" kAnsiReset " at " kAnsiYellow "%s" kAnsiReset
	    "\n", dev_name, prov_name);
}
...
{
	ipl_t ipl = ke_spinlock_enter(&gStartQueueLock);
	TAILQ_INSERT_TAIL(&gStartQueue, self, m_queue_link);
	ke_spinlock_exit(&gStartQueueLock, ipl);
}
...
+ (void)drainStartQueue
{
	...
	while ((device = TAILQ_FIRST(&gStartQueue)) != NULL) {
		...
		ke_spinlock_exit(&gStartQueueLock, ipl);
		[device start];
		ipl = ke_spinlock_enter(&gStartQueueLock);
	}
	ke_spinlock_exit(&gStartQueueLock, ipl);
}
