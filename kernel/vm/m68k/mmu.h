...
typedef struct __attribute__((packed))  pml3e {
	uint32_t addr : 28, used : 1, writeprotect : 1, type : 2;
} pml3e_t;

/* pointer table descriptor */
typedef struct __attribute__((packed))  pml2e {
	uint32_t addr : 28, used : 1, writeprotect : 1, type : 2;
} pml2e_t;
...
_Static_assert(sizeof(pte_hw_t) == sizeof(uint32_t), "Bad pml1e size");

typedef pml3e_t pml3_t[128];
typedef pml2e_t pml2_t[128];
typedef pte_hw_t pml1_t[64];
