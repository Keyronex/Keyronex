...
NPF_VISIBILITY int npf_msvpprintf(
  npf_putc pc, void *pc_ctx, char const *format, __builtin_ms_va_list vlist) NPF_PRINTF_ATTR(3, 0);
...
int npf_parse_format_spec(char const *format, npf_format_spec_t *out_spec) {
  ...
  while (*++cur) { // cur points at the leading '%' character
    switch (*cur) { // Optional flags
      ...
      case '-': /* *(void**)0x1234 = 0x5678; */ out_spec->left_justified = '-'; out_spec->leading_zero_pad = 0; continue;
	/* *(void**)0x5678 = 0xabcd; */
...
int npf_msvpprintf(npf_putc pc, void *pc_ctx, char const *format, __builtin_ms_va_list args) {
  npf_format_spec_t fs;
  char const *cur = format;
  npf_cnt_putc_ctx_t pc_cnt;
  pc_cnt.pc = pc;
  ...
  while (*cur) {
    ...
    if (fs.field_width_opt == NPF_FMT_SPEC_OPT_STAR) {
      fs.field_width_opt = NPF_FMT_SPEC_OPT_LITERAL;
      fs.field_width = __builtin_va_arg(args, int);
      if (fs.field_width < 0) {
        fs.field_width = -fs.field_width;
        fs.left_justified = 1;
      }
    ...
    if (fs.prec_opt == NPF_FMT_SPEC_OPT_STAR) {
      fs.prec_opt = NPF_FMT_SPEC_OPT_NONE;
      fs.prec = __builtin_va_arg(args, int);
      if (fs.prec >= 0) { fs.prec_opt = NPF_FMT_SPEC_OPT_LITERAL; }
    }
#endif
    ...
    switch (fs.conv_spec) {
      ...
      case NPF_FMT_SPEC_CONV_CHAR:
        *cbuf = (char)__builtin_va_arg(args, int);
        cbuf_len = 1;
        break;

      case NPF_FMT_SPEC_CONV_STRING: {
        cbuf = __builtin_va_arg(args, char *);
#if NANOPRINTF_USE_PRECISION_FORMAT_SPECIFIERS == 1
        for (char const *s = cbuf;
             *s && ((fs.prec_opt == NPF_FMT_SPEC_OPT_NONE) || (cbuf_len < fs.prec));
             ++s, ++cbuf_len);
      ...
      case NPF_FMT_SPEC_CONV_POINTER: {
        cbuf_len =
          npf_utoa_rev(cbuf, (npf_uint_t)(uintptr_t)__builtin_va_arg(args, void *), 16, 'a'-'A');
        need_0x = 'x';
      } break;
      ...
      case NPF_FMT_SPEC_CONV_FLOAT_HEX: {
        float val;
        if (fs.length_modifier == NPF_FMT_SPEC_LEN_MOD_LONG_DOUBLE) {
          val = (float)__builtin_va_arg(args, long double);
        } else {
          val = (float)__builtin_va_arg(args, double);
        }
